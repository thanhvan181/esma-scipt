 ...
 async render() {
    ...
    ${sideBar.render()}
      ...
      <div class="flex items-center justify-between py-7 px-10">
        <div>
          <h1 class="text-2xl font-semibold leading-relaxed text-gray-800">Category</h1>
        ...
        >
          ...
          <a href="/admin/category/add"class="text-sm font-semibold tracking-wide">Add Category</a>
        </button>
      </div>
      ...
      <table class="w-full border-b border-gray-200">
        ...
        ${CateList.map((elCate, index) => {
          ...
          >
            ...
            <td class="font-medium text-center">${elCate.title}</td>
  ...
  afterRender() {
      const btnDeleteCate = document.querySelectorAll(".btn")
     btnDeleteCate.forEach((btnCate) => {
       // Lấy giá trị ID thông qua thuộc tính data-id của button
       const { id } = btnCate.dataset;
       console.log("Id", id);
